    ...
    if carpet_width and carpet_length:
        ...
        if item_id in list(bag.keys()):
            if carpet_area in bag[item_id]['item_measurements'].keys():
                # If carpet with specified area is 
                # already in bag, update quantity
                bag[item_id]['item_measurements'][carpet_area] += quantity
                messages.success(request, f'Updated {product.name} quantity to {bag[item_id]["item_measurements"][carpet_area]}')
            else:
        ...
        if item_id in list(bag.keys()):
            # Increase quantity of item in bag 
            # that does not require size to be specified
            bag[item_id] += quantity
            messages.success(request, f'Updated {product.name} quantity to {bag[item_id]}')
        else:
            # Add item to bag that does not require size to be specified
            bag[item_id] = quantity
            messages.success(request, f'Added {product.name} to your bag')
...
def remove_from_bag(request, item_id):
    ...
    try:
        ...
        messages.error(request, f'Error removing item {product.name} from bag')
